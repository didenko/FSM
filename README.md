...
To load the code in _Visual Studio_, double click on the `fsm.sln` file. The project has only `Debug | x64` configuration. Before running tests, make sure that _X64_ architecture is selected at the menu `TEST`>`Test Settings`>`Default Processor Architecture`>`X64`. Them you can build and run tests (`Ctrl+R`, `A` key sequence).
